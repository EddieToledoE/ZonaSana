...
function SeleccionProductos({ onProductosSeleccionados }) {
  ...
  const [productoentero, setProductoentero] = useState(null);
  const [productoSeleccionado, setProductoSeleccionado] = useState(null);
  const [cantidad, setCantidad] = useState(0); // Valor inicial de la cantidad
  ...
  useEffect(() => {
    const obtenerProductos = async () => {
      try {
        const response = await axios.get("/api/auth/producto/stock");
  ...
  const agregarProducto = () => {
    if (!productoSeleccionado) {
      Swal.fire({
        ...
        text: "Debes de seleccionar un producto",
      });
      return;
    }
    ...
    if (productoSeleccionado && cantidad) {
      ...
      if (maximo < cantidad) {
        Swal.fire({
          ...
          text: "Esa cantidad supera el stock disponible :" + maximo,
        });
        return;
      }
  ...
  const handleAutocompleteChange = (event, newValue) => {
    ...
    setMaximo(newValue ? newValue.cantidad_stock : 0);
    setMonto(newValue ? newValue.precio_venta * cantidad : 0);
    setGanancia((prevGanancia) => {
      if (!newValue) return 0;

      const ganancia =
        (newValue.precio_venta - newValue.precio_costo) * cantidad;

      return ganancia;
    });
    console.log(monto, ganancia);
  };
    ...
    <div className="Carrito-Seccion" style={{border:'1px dashed  #383e49',width:'80%',overflow:'auto'}}>
      <div className="Agregar_Carrito">
        ...
        <Autocomplete
          ...
          onChange={(event, value) => handleAutocompleteChange(event, value)} // Maneja el cambio de valor en Autocomplete
          value={productoentero}
          renderInput={(params) => (
        ...
        <input
          ...
          id="cantidad"
          value={cantidad}
          max={maximo} // stock máximo del producto
